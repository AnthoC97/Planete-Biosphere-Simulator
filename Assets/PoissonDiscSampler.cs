...
public class PoissonDiscSampler : MonoBehaviour
{
    ...
    public void StartSampling(int seed)
    {
        ...
        Vector3 point = new Vector3(
            point.x = Mathf.Lerp(regionWidth * 0.4f, regionWidth * 0.6f, (float)random.NextDouble()),
            point.y = Mathf.Lerp(regionHeight * 0.4f, regionHeight * 0.6f, (float)random.NextDouble()),
            0);
        if (dimensions == 3)
            point.z = Mathf.Lerp(regionLength * 0.4f, regionLength * 0.6f, (float)random.NextDouble());
    ...
    IEnumerator Fill()
    {
        ...
        while (activeList.Count > 0)
        {
            ...
            int randomIndex = (int)Mathf.Lerp(0, activeList.Count, (float)random.NextDouble());
            var randomPoint = activeList[randomIndex];

            bool found = false;
            for (int k = 0; k < rejectionLimit; k++)
            {
                var length = Mathf.Lerp(minDistance, 2 * minDistance, (float)random.NextDouble());
                var angle = (int)Mathf.Lerp(0, 360, (float)random.NextDouble());
                Vector3 sample = Quaternion.AngleAxis(angle, Vector3.forward) * Vector3.up;
                if (dimensions == 3)
                {
                    angle = (int)Mathf.Lerp(0, 360, (float)random.NextDouble());
